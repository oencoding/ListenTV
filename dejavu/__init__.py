...
class Dejavu(object):
    ...
    def get_fingerprinted_songs(self):
        ...
        self.songnames_set = set() # songname_set
        for song in self.songs:
            song_hash = song[Database.FIELD_FILE_SHA1]
            song_name = song[Database.FIELD_SONGNAME]
            self.songhashes_set.add(song_hash)
            printed = song[Database.FIELD_FINGERPRINTED]
            # print type(printed),printed,song_name

            if printed == 1:
                self.songnames_set.add(song_name)

    def isSongFingerPrinted(self,song_name):
